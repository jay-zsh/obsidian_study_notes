...
### 多态应用
>多态（Polymorphism）是面向对象编程的三大核心特性之一，指**同一操作作用于不同对象时，会产生不同的行为**。其核心是通过抽象和接口实现代码的灵活性。
...
public class WordUtils {
    public static String longest(List61B<String> list) {
        ...
        for (int i=1; i<list.size(); i++) {
            ...
            if (current.length() > max.length()) {//length
                max = current;
            }
        }
        return max;
...
1. 继承或实现关系：子类继承父类，或实现接口(`public interface List61<item>`。
2. 方法重写：子类需覆盖父类/接口的方法(`public class SLList<T> implemnets List61B<T>。
3. 向上转型：父类引用指向子类对象（如 `List61B<String> alist = new Alist<>()`）。
